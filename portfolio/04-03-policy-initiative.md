...
1. ポリシータグをクリック
2. 「ポリシー定義の追加」をクリックする。
3. 一覧から `Require a tag on resources` を選択する。
4. 「追加」をクリックする。
5. 再度「ポリシーの追加」をクリックする。
6. 一覧から `Require a tag and its value on resources` を選択する。
6. 「追加」をクリックする。
...
1. 「ポリシー パラメーター」タブをクリックする。
2. `Require a tag on resources` の値にタグ名 `Environment` を入力する。
3. `Require a tag and its value on resources` の値にタグ名 `Environment` を入力する。
4. タグの値に `PolicyTest` を入力する。
5. 「次へ」をクリックする。
6. 「レビューと作成」→「作成」をクリックする。
